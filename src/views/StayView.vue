  ...
  <div class="">
    <div class="bg-white w-full py-3 shadow-lg fixed top-[74px] z-10">
      <div class="px-16 flex items-center">
        <button v-show="scrollPage > 0" @click="scrollLeft1" class="shadow-lg w-10 h-10 rounded-full bg-gray-100 text-black mx-3">
          <i class="fa-solid fa-angle-left"></i>
        </button>
        <div class="w-[700px] flex text-nowrap bg-transparent overflow-hidden overflow-x-scroll no-scrollbar cursor-pointer">
          <Button class="transtion-all duration-500"  v-for="(item , index) in useCatagories.items" :key="index" :item="item" :style="{ transform: `translateX(${scrollLeft}px)` }"/>
        </div>
        <button @click="scrollRight" class="shadow-lg w-10 h-10 rounded-full bg-gray-100 text-black">
          <i class="fa-solid fa-angle-right"></i>
        </button>
        <button class="hover:shadow-lg px-5 py-3 ml-5 rounded-lg duration-700 font-semibold border">
